...
#define RADIO_SPI2_NSS_PORT GPIOA
#define RADIO_SPI2_NSS_PIN GPIO_PIN_4

struct rt_spi_device *rf_4068_radio_spi_init(void)
{
    rt_err_t res;
    static struct rt_spi_device *radio_spi_device;
    rt_hw_spi_device_attach("spi1", "rf_4068", RADIO_SPI2_NSS_PORT, RADIO_SPI2_NSS_PIN);
    radio_spi_device = (struct rt_spi_device *)rt_device_find("rf_4068");
    if (!radio_spi_device)
    {
        LOG_D("spi_device_attach failed! cant't find rf_4068 device!\n");
        return RT_NULL;
    }
    struct rt_spi_configuration cfg;
    cfg.data_width = 8;
    cfg.mode = RT_SPI_MASTER | RT_SPI_MODE_0 | RT_SPI_MSB; /* SPI Compatible: Mode 0. */
    cfg.max_hz = 1 * 100000;             /* max 200k */
